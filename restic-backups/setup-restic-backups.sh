...
read -r -p "Have you updated A) setup-restic-env.private.sh, B) updated the restic version in the curl command herein, and C) run restic init after sourcing setup-restic-env.private.sh file ? [y/N] " response
case "$response" in
    [yY][eE][sS]|[yY]) 
        echo "Continuing..."
        ;;
...
sudo setcap cap_dac_read_search=+ep /opt/restic-backup/bin/restic
